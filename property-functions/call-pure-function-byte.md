---
title: Call Pure Function (Byte)
---

# Call Pure Function (Byte)

**Plugin:** StateTreeToolsCore
**Category:** Utilities \| Byte

Calls a `BlueprintPure` function on an actor (or one of its components) at runtime and exposes the return value as a bindable `byte` (`uint8`) output. Use this to retrieve a byte or enum value computed by Blueprint pure logic directly within the StateTree binding system.
...
#### FunctionName
The `BlueprintPure` function to call. Only functions with a return value compatible with `byte` are shown in the dropdown.

#### ReturnValueName
The output parameter of the function to read. This is usually `ReturnValue` for single-output functions.

### Value
The `byte` result returned by the called function.
